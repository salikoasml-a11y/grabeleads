...
const leadsInventory = require('../data/leads-inventory.json')
...
function pickLeads(count) {
  const shuffled = [...leadsInventory].sort(() => Math.random() - 0.5)
  return shuffled.slice(0, Math.min(count, shuffled.length))
}

// ─── Lead storage (Vercel KV) ────────────────────────────────────────────────

async function readLeads() {
  try { return (await kv.get('leads')) ?? [] } catch { return [] }
}
...
async function saveLead(lead) {
  ...
  try {
    ...
    // KV not configured — lead is still emailed to owner
  }
  return newLead
}
...
app.post('/api/leads', async (req, res) => {
  ...
  const lead = await saveLead({ name: name.trim(), email: email.trim(), company: company.trim(), plan: 'free', source: 'form' })
  await notifyOwner(lead)
...
app.post('/api/create-checkout-session', async (req, res) => {
  ...
  const origin = CLIENT_URL || `https://${req.headers.host}`
